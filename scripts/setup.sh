...
echo "2. 确保上面所有前置条件为 ✅"
echo "3. 启动监控（三选一）："
echo "   方案A（企微全自动）: export KNOCKET_WORK_DIR=$(pwd) && nohup python3 ${SKILL_DIR}/scripts/wecom_auto.py > /dev/null 2>&1 &"
echo "   方案B（TG人工优先）: export KNOCKET_WORK_DIR=$(pwd) && nohup python3 ${SKILL_DIR}/scripts/telegram_human.py > /dev/null 2>&1 &"
echo "   方案C（微信人工优先）⭐: export KNOCKET_WORK_DIR=$(pwd) && nohup python3 ${SKILL_DIR}/scripts/wechat_human.py > /dev/null 2>&1 &"
echo ""
